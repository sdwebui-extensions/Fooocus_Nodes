...
import fooocus_modules.sample_hijack
import fooocus_ldm_patched.modules.samplers
import fooocus_ldm_patched.modules.latent_formats

from fooocus_ldm_patched.modules.sd import load_checkpoint_guess_config
from fooocus_ldm_patched.contrib.external import VAEDecode, EmptyLatentImage, VAEEncode, VAEEncodeTiled, VAEDecodeTiled, \
    ControlNetApplyAdvanced
from fooocus_ldm_patched.contrib.external_freelunch import FreeU_V2
from fooocus_modules.lora import match_lora
from fooocus_ldm_patched.modules.lora import model_lora_keys_unet, model_lora_keys_clip
from fooocus_ldm_patched.contrib.external_model_advanced import ModelSamplingDiscrete
...
class StableDiffusionModel:
    ...
    def refresh_loras(self, loras):
        ...
        for lora_filename, weight in loras_to_load:
            lora_unmatch = fooocus_ldm_patched.modules.utils.load_torch_file(lora_filename, safe_load=False)
...
def load_controlnet(ckpt_filename):
    return fooocus_ldm_patched.modules.controlnet.load_controlnet(ckpt_filename)
...
def load_model(ckpt_filename, vae_filename=None):
    import folder_paths
    unet, clip, vae, vae_filename, clip_vision = load_checkpoint_guess_config(ckpt_filename, embedding_directory=folder_paths.get_folder_paths("embeddings"), vae_filename_param=vae_filename)
...
def get_previewer(model):
    ...
    is_sdxl = isinstance(model.model.latent_format, fooocus_ldm_patched.modules.latent_formats.SDXL)
    vae_approx_filename = folder_paths.get_full_path('vae_approx','xlvaeapp.pth' if is_sdxl else 'vaeapp_sd15.pth')
...
def ksampler(model, positive, negative, latent, seed=None, steps=30, cfg=7.0, sampler_name='dpmpp_2m_sde_gpu',
    ...
    disable_pbar = False
    fooocus_modules.sample_hijack.current_refiner = refiner
    fooocus_modules.sample_hijack.refiner_switch_step = refiner_switch
    fooocus_ldm_patched.modules.samplers.sample = fooocus_modules.sample_hijack.sample_hacked

    try:
        samples = fooocus_ldm_patched.modules.sample.sample(model,
        ...
        fooocus_modules.sample_hijack.current_refiner = None
