...
import fooocus_modules.inpaint_worker
import fooocus_extras.vae_interpose as vae_interpose
from fooocus_extras.expansion import FooocusExpansion

from fooocus_ldm_patched.modules.model_base import SDXL, SDXLRefiner
from fooocus_modules.sample_hijack import clip_separate
...
def refresh_base_model(name, vae_name=None):
    ...
    vae_filename = None
    if vae_name is not None and vae_name not in ["Baked VAE", "Baked-VAE"]:
        vae_filename = folder_paths.get_full_path("vae", vae_name)
...
# refresh_everything(
#     refiner_model_name=fooocus_modules.config.default_refiner_model_name,
#     base_model_name=fooocus_modules.config.default_base_model_name,
#     loras=get_enabled_loras(fooocus_modules.config.default_loras)
# )
...
def calculate_sigmas_all(sampler, model, scheduler, steps):
    from fooocus_ldm_patched.modules.samplers import calculate_sigmas_scheduler
...
def process_diffusion(positive_cond, negative_cond, steps, switch, width, height, image_seed, callback, sampler_name, scheduler_name, latent=None, denoise=1.0, tiled=False, cfg_scale=7.0, refiner_swap_method='joint', disable_preview=False):
    ...
    fooocus_modules.patch.BrownianTreeNoiseSamplerPatched.global_init(
    ...
    if refiner_swap_method == 'vae':
        fooocus_modules.patch.patch_settings[os.getpid()].eps_record = 'vae'

        if fooocus_modules.inpaint_worker.current_task is not None:
            fooocus_modules.inpaint_worker.current_task.unswap()
        ...
        noise_mean = torch.mean(fooocus_modules.patch.patch_settings[os.getpid()].eps_record, dim=1, keepdim=True)

        if fooocus_modules.inpaint_worker.current_task is not None:
            fooocus_modules.inpaint_worker.current_task.swap()
    ...
    fooocus_modules.patch.patch_settings[os.getpid()].eps_record = None
    return images
