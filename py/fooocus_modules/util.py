...
import fooocus_modules.sdxl_styles
...
def extract_styles_from_prompt(prompt, negative_prompt):
    ...
    for style_name, (style_prompt, style_negative_prompt) in fooocus_modules.sdxl_styles.styles.items():
    ...
    if prompt != '':
        if real_prompt != '':
            extracted.append(fooocus_modules.sdxl_styles.fooocus_expansion)
        else:
...
def apply_wildcards(wildcard_text, rng, i, read_wildcards_in_order) -> str:
    for _ in range(fooocus_modules.config.wildcards_max_bfs_depth):
        ...
        for placeholder in placeholders:
            try:
                matches = [x for x in fooocus_modules.config.wildcard_filenames if os.path.splitext(os.path.basename(x))[0] == placeholder]
                words = open(os.path.join(fooocus_modules.config.path_wildcards, matches[0]), encoding='utf-8').read().splitlines()
